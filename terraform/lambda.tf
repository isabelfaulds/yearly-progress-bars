...
resource "aws_s3_bucket_object" "sync_gcal_tasklist" {
  bucket = aws_s3_bucket.pbars_lambdas_bucket.bucket
  source = "../backend/cal-sync/gapi-list/gapi-tasklists.zip"
  etag = filemd5("../backend/cal-sync/gapi-list/gapi-tasklists.zip")
...
resource "aws_lambda_permission" "allow_apigateway_gcal_tasklist" {
  ...
  principal     = "apigateway.amazonaws.com"
  source_arn    = "arn:aws:execute-api:us-west-1:${data.aws_caller_identity.current.account_id}:${var.api_id}/*/GET/calendar/sync/gcal/tasks/list"
}